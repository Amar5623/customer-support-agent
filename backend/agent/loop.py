...
══ TOOL DISCIPLINE ══
Before calling any tool, reason silently:
...
When a customer asks about "my order" without specifying which one:
  ...
  Step 4 → Call get_order_details(order_id) with the confirmed ID.

When a customer wants to change the delivery date:
  - Must have a confirmed order_id before calling change_delivery_date.
  - If customer says "sooner" / no specific date: call get_order_details first,
...
def _build_history_summary(old_messages: list[Message]) -> Message | None:
    ...
    for msg in old_messages:
        if msg.role == Role.user:
            ...
            # Either a tool-call encoding or the final reply
            ...
            # Extract key facts from tool results to keep in the summary
            try:
    ...
    # Injected as a user message with a clear marker so the LLM treats it as context
...
async def run_agent(
    ...
    # ── History trimming ──────────────────────────────────────────────────────
    #
    # Split history into turns. Keep the most recent VERBATIM_TURNS verbatim.
    # Compress anything older into a single summary message.
    # This prevents token cost from growing unbounded across a long conversation.
    ...
    # ── Log total estimated input size ───────────────────────────────────────
    ...
    logger.info(
        ...
        f"total estimate: ~{prompt_tokens + history_tokens} tokens"
    )
